...
const DocumentList = () => {
  ...
  const [viewMode, setViewMode] = useState(() => sessionStorage.getItem('docViewMode') || 'grid');
  ...
  const applyFilters = () => {
    ...
    const defaultCategories = new Set(['license','certificate','permit','insurance','contract','bank','medical','miscellaneous','other']);
    if (filters.category !== 'all') {
      if (filters.category === 'other') {
        ...
        filtered = filtered.filter(doc => {
          ...
          const linkedMatch = doc.categoryId && doc.categoryId.name === filters.category;
    ...
    if (sortBy === 'newest') {
      filtered.sort((a, b) => new Date(b.uploadedAt || b.createdAt) - new Date(a.uploadedAt || a.createdAt));
  ...
  const formatExpiryContext = (expiryDate) => {
    ...
    if (daysUntil < 0) return { text: `Expired: ${formatted}`, status: 'expired' };
    if (daysUntil <= 30) return { text: `Expires: ${formatted}`, status: 'warning' };
  ...
  const totalPages = Math.ceil((filteredDocuments?.length || 0) / pagination.itemsPerPage);
          ...
          {viewMode === 'grid' ? (
            <div className="documents-grid">
              {paginatedDocuments.map(doc => {
                ...
                const catColor = doc.categoryId?.color || getCategoryColor(doc.category);
                ...
                return (
                  ...
                  >
                    ...
                    >
                      ...
                      <div className={`card-expiry expiry-${expiryContext.status}`}>
                        {expiryContext.text}
                      </div>
                    </div>
              ...
              {paginatedDocuments.map(doc => {
                ...
                return (
                  <div 
                    ...
                    className="list-item"
                  >
                    <div className="list-main" onClick={() => navigate(`/documents/${doc._id}`)}>
                      ...
                      <div className="list-meta">
                        ...
                        <span className={`meta-expiry expiry-${expiryContext.status}`}>
                          {expiryContext.text}
                        </span>
                      </div>
                    </div>
